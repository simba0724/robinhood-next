...
import {KET_ADDRESS, MEDIA_ADDRESS} from 'config';
...
const MediaDetail = ({id}: any) => {
  ...
  const [removeAskLoading, setRemoveAskLoading] = useState<boolean>(false);
  ...
  useEffect(() => {
    const loadContractData = async () => {
      ...
      setContractData({
        ...
        bids,
        creator,
        isOwner: account && account === owner_address,
      });
      setLoading(false);
    };
    loadContractData();
  ...
  }, [contractData, askLoading, removeAskLoading]);
  ...
  const removeAsk = async (e: any) => {
    e.preventDefault();
    setRemoveAskLoading(true);
    const transaction = await mediaContract?.removeAsk(
      contractData?.media?.media_id,
    );
    await transaction?.wait(1);
    setRemoveAskLoading(false);
  ...
  const acceptBid = async (media: any) => {
    try {
      ...
      const transaction = await mediaContract?.acceptBid(media?.media_id, bid); // KNOWN ISSUE: return all the bids except accepted bids
      await transaction?.wait(2);
      // should confirm if bidding is finished here.
      // await axios.post(
      //   `/api/media/removebids`,
      //   JSON.stringify({
  ...
  return (
    <Layout
      title={`${contractData?.media?.metadata?.title} | RBH Marketplace`}>
      <>
        <div className="py-6">
                  ...
                  <div className="flex items-center space-x-4 my-4">
                    <div>
                      <div className="rounded-lg flex py-2">
                        ...
                        <span className="text-indigo-400 mr-1 mt-2">RBH</span>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex py-4 space-x-4">
                    {contractData?.isOwner && (
                      <button
                        ...
                        disabled={askLoading}
                        className={`h-14 px-6 py-2 outline-none font-semibold rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white ${askLoading && "cursor-not-allowed"}`}>
                        {!askLoading ? "Set Ask" : "Processing..."}
                      </button>
                    )}
                    {contractData?.isOwner && amount && token && (
                      <button
                        ...
                        disabled={removeAskLoading}
                        className={`h-14 px-6 py-2 outline-none font-semibold rounded-xl bg-red-600 hover:bg-red-500 text-white ${removeAskLoading && "cursor-not-allowed"}`}>
                        {!removeAskLoading ? "Remove Ask" : "Processing..."}
                      </button>
                    )}
                    {!contractData?.isOwner && amount && token && (
                      <button
                        ...
                        disabled={bidLoading}
                        className={`h-14 px-6 py-2 outline-none font-semibold rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white ${bidLoading && "cursor-not-allowed"}`}>
                        {!bidLoading ? "Bid" : "Processing..."}
                      </button>
                    )}
                  </div>
                )}
